...
@MainActor
private func getTopWindow() -> UIWindow? {
    // Get all connected window scenes that are active
    let windowScenes = UIApplication.shared.connectedScenes
        .compactMap { $0 as? UIWindowScene }
        .filter { $0.activationState == .foregroundActive }

    // Get the topmost visible window in those scenes
    return windowScenes
        .flatMap { $0.windows }
...
public func emitConfetti(with confettiView: ConfettiView) {
    ...
    // Add directly to the window so it appears above everything, including sheets
