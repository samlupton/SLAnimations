...
public extension Plume.Cell.Lifetime {
    /// Near-instant feedback (tap, impact, micro-response).
    static let instant: Self = .init(base: 0.25, range: 0.10)

    /// Extremely short-lived motion, almost like a blink.
    static let flash: Self = .init(base: 0.5, range: 0.15)
}
...
public extension Plume.Cell.Lifetime {
    ...
    static let short: Self = .init(base: 0.9, range: 0.25)

    /// Default balanced behavior for most UI effects.
    static let normal: Self = .init(base: 2.5, range: 0.8)
}
...
public extension Plume.Cell.Lifetime {
    ...
    static let long: Self = .init(base: 6.0, range: 2.0)

    /// Extended visible lifecycle for expressive or celebratory effects.
    static let extended: Self = .init(base: 10.0, range: 3.0)
}
...
public extension Plume.Cell.Lifetime {
    ...
    static let persistent: Self = .init(base: 15.0, range: 5.0)

    /// Slow atmospheric behavior for ambient layers.
    static let ambient: Self = .init(base: 25.0, range: 8.0)

    /// Very long-lived behavior for continuous or system-like motion.
    static let continuous: Self = .init(base: 45.0, range: 15.0)
}
